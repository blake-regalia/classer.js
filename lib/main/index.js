...
const bind_property = (w_operator, s_property, d_ipc_node, k_instance) => {
	...
	if('function' === typeof h_property_descriptor.value) {
		...
		Object.defineProperty(w_operator, s_property,
	...
	// property is something else
		...
		Object.defineProperty(w_operator, s_property, h_property_descriptor);
	}
};

// set prototype of operator to instance and override function's own properties
const bind_operator_proxy = (w_operator, k_instance) => {

	// operator is function
	if('function' === typeof w_operator) {
		// bind instance to operator
		w_operator = Function.prototype.bind.apply(w_operator, [k_instance]);
	}

	// set prototype of operator to the instance itself
	Object.setPrototypeOf(w_operator, k_instance);
	...
	a_inverse_prototype_chain.forEach((d_ipc_node) => {
		...
		if(d_ipc_node.hasOwnProperty('length')) {
			if(Array.isArray(w_operator)) throw `cannot use an array operator AND define '.length' on class instance`;
			bind_property(w_operator, 'length', d_ipc_node, k_instance);
		}
		// override name property
		if(d_ipc_node.hasOwnProperty('name')) {
			bind_property(w_operator, 'name', d_ipc_node, k_instance);
		}
	});
...
local.export = function(y_class, w_operator, h_static={}) {
	...
	let f_cover = function(...a_args) {
		...
		if(w_operator) {
			...
			// bind operator proxy
			return bind_operator_proxy(w_operator, k_instance);
		}
...
local.exportAsync = function(y_class, w_operator, h_static={}) {
	...
	let f_cover = function(...a_args) {
		...
		if('function' === typeof f_okay_instance) {
			...
			const f_okay_constructor = (w_override_operator) => {

				// use operator
				if(w_override_operator || w_operator) {
					// bind operator proxy
					f_okay_instance(bind_operator_proxy(w_override_operator || w_operator, k_instance));
				}
				// no operator
				else {
					// all done :)
